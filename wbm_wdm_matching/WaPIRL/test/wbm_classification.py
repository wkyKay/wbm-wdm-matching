...
def main_worker(local_rank: int, config: object):
    ...
    train_set = WM811K('./data/wm811k/wbm(10*10)/labeled/train/',
                       transform=train_transform,
                       proportion=config.label_proportion,
                       decouple_input=config.decouple_input)
    valid_set = WM811K('./data/wm811k/wbm(10*10)/labeled/valid/',
                       transform=test_transform,
                       decouple_input=config.decouple_input)
    test_set  = WM811K('./data/wm811k/wbm(10*10)/labeled/test/',
                       transform=test_transform,
                       decouple_input=config.decouple_input)
    ...
    backbone = WBM_Encoder(in_channels=in_channels, output_dim=512)
    classifier = LinearClassifier(in_channels=512, num_classes=num_classes)
