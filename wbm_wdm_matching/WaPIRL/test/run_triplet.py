...
def train_cross_modal():
    ...
    print(config.proportion)
    train_set = torch.utils.data.ConcatDataset([
        WM811KForTriplet(
            wbm_root ='./data/wm811k/paired_data/train/wbm/', 
            wdm_root = './data/wm811k/paired_data/train/wdm/', 
            proportion=config.proportion,
            **data_kwargs),
    ])
    valid_set = torch.utils.data.ConcatDataset([
        WM811KForTriplet(
            wbm_root ='./data/wm811k/paired_data/val/wbm', 
            wdm_root="./data/wm811k/paired_data/val/wdm/", 
            proportion=config.proportion,
            **data_kwargs),
    ])
    test_set = torch.utils.data.ConcatDataset([
        WM811KForTriplet(
            wbm_root='./data/wm811k/paired_data/test/wbm/',
            wdm_root='./data/wm811k/paired_data/test/wdm/',
            proportion=config.proportion,
            **data_kwargs),
    ])
    ...
    train_loader = DataLoader(train_set, config.batch_size, num_workers=config.num_workers, shuffle=True, pin_memory=False)
    val_loader = DataLoader(valid_set, config.batch_size, num_workers=config.num_workers, shuffle=True, pin_memory=False)
    test_loader = DataLoader(test_set, config.batch_size, num_workers=config.num_workers, shuffle=True, pin_memory=False)
   # 使用示例
    ...
    model = CrossModalRetrievalModel(
        ...
        embedding_dim=128,
        freeze_encoder=True,  # 冻结encoder，只训练投影头
    )
    ...
    #     mining='semi-hard'
    # )
    # triplet_loss = InfoNCELoss(
    #     temperature=0.1
    # )
    triplet_loss = MultiSimilarityLoss()
    ...
    # 检索评估
    test_recall = trainer.evaluate(test_loader, k_list=[1, 3, 5])
    logger.info(f"Test Recall@1: {test_recall['recall@1']:.4f}")
    logger.info(f"Test Recall@3: {test_recall['recall@3']:.4f}")
    logger.info(f"Test Recall@5: {test_recall['recall@5']:.4f}")
