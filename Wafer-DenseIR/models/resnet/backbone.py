...
def downsample1x1(in_channels: int, out_channels: int, stride: int = 2):
    return nn.Sequential(conv1x1(in_channels, out_channels, stride), nn.BatchNorm2d(out_channels))
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1, **kwargs):
        ...
        self.conv1 = conv3x3(in_channels, out_channels * width, stride=stride)
        self.bnorm1 = nn.BatchNorm2d(out_channels * width)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(out_channels * width, out_channels, stride=1)
        self.bnorm2 = nn.BatchNorm2d(out_channels)
        self.relu2 = nn.ReLU(inplace=True)
        self.downsample = downsample1x1(in_channels, out_channels, stride) if stride != 1 else None

    def forward(self, x):
        identity = self.downsample(x) if self.downsample is not None else x
        out = self.relu1(self.bnorm1(self.conv1(x)))
...
class BottleNeck(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1, **kwargs):
        ...
        width = kwargs.get('width', 1)
        self.conv1 = conv1x1(in_channels, out_channels * width)
        self.bnorm1 = nn.BatchNorm2d(out_channels * width)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(out_channels * width, out_channels * width, stride)
        self.bnorm2 = nn.BatchNorm2d(out_channels * width)
        self.relu2 = nn.ReLU(inplace=True)
        self.conv3 = conv1x1(out_channels * width, out_channels * self.expansion)
        ...
        self.downsample = downsample1x1(in_channels, out_channels * self.expansion, stride) if needs_projection else None

    def forward(self, x):
        identity = self.downsample(x) if self.downsample is not None else x
        out = self.relu1(self.bnorm1(self.conv1(x)))
        out = self.relu2(self.bnorm2(self.conv2(out)))
...
class ResNetBackbone(BackboneBase):
    ...
    def __init__(self, layer_config: dict, in_channels: int = 2):
        ...
        self.first_conv = layer_config.get('first_conv', 3)
        self.width = layer_config.get('width', 1)
        self.layers = self.make_layers(layer_config, in_channels, width=self.width, first_conv=self.first_conv)
    ...
    def make_layers(cls, layer_cfg: dict, in_channels: int, **kwargs):
        ...
        if kwargs['first_conv'] == 3:
            block0 = nn.Sequential(collections.OrderedDict([
        ...
        for i, channels in enumerate(layer_cfg['channels']):
            stride = layer_cfg['strides'][i]
            layers.add_module(f'block{i + 1}', Block(in_channels, channels, stride=stride, **kwargs))
