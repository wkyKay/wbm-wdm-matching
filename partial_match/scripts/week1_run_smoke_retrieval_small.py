# -*- coding: utf-8 -*-
"""
Week 1: Run Smoke Baseline Retrieval - Small Version
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run smoke baseline retrieval for Week 1 (small version)")
    ...
    parser.add_argument("--num-samples", type=int, default=200, 
                        help="Number of samples to use for small run")
    ...
    # Use a small subset of the validation split
    sample_ids = splits[args.split]
    num_samples = min(args.num_samples, len(sample_ids))
    sample_ids_small = sample_ids[:num_samples]
    split_binary_maps = binary_maps[sample_ids_small]
    
    print(f"Running {args.method} baseline on {num_samples} samples...")
