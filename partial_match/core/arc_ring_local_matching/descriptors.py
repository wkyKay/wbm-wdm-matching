...
def _shape_descriptor(
    ...
    token["descriptor_parts"] = {
        ...
        "zernike_normalization": "centroid_radius",
    }
...
def _zernike_descriptor(token: Dict, out_size: int = 48, degree: int = 8) -> np.ndarray:
    values = _centroid_radius_canvas(token, out_size=out_size)
    ...
    unit_disk = rho <= 1.0
    values = values * unit_disk.astype(np.float32)
...
def _centroid_radius_canvas(token: Dict, out_size: int) -> np.ndarray:
    ...
    rows = pixels[:, 0]
    cols = pixels[:, 1]
    centroid_row = float(token.get("centroid_row", float(rows.mean())))
    centroid_col = float(token.get("centroid_col", float(cols.mean())))
    rel_rows = rows - centroid_row
    rel_cols = cols - centroid_col
    pixel_outer_radius = np.sqrt((np.abs(rel_rows) + 0.5) ** 2 + (np.abs(rel_cols) + 0.5) ** 2)
    norm_radius = float(pixel_outer_radius.max()) if pixel_outer_radius.size else 0.0
    if norm_radius <= 1e-8:
        return np.zeros((out_size, out_size), dtype=np.float32)

    canvas = np.zeros((out_size, out_size), dtype=np.float32)
    center = (out_size - 1) / 2.0
    canvas_radius = max(center, 1.0)
    half_extent = max(0.5 * canvas_radius / norm_radius, 0.5)
    canvas_rows = center + rel_rows * canvas_radius / norm_radius
    canvas_cols = center + rel_cols * canvas_radius / norm_radius
    for row, col in zip(canvas_rows, canvas_cols):
        r0 = max(int(np.floor(row - half_extent)), 0)
        r1 = min(int(np.ceil(row + half_extent)), out_size - 1)
        c0 = max(int(np.floor(col - half_extent)), 0)
        c1 = min(int(np.ceil(col + half_extent)), out_size - 1)
        canvas[r0:r1 + 1, c0:c1 + 1] = 1.0
    return canvas
