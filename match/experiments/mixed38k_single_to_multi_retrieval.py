"""Mixed38K single-class query to multi-label map retrieval experiment.

For each single-label query map, retrieve real Mixed38K candidate maps.
Positive candidates are multi-label maps that contain the query class.
Negative candidates are real maps that do not contain the query class.
Metrics include precision@k, recall@k, hit@k, AP, nDCG, MRR, and
positive-rank summaries.
"""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Mixed38K single-class query to multi-label retrieval experiment.")
    ...
    parser.add_argument("--trials-per-class", type=int, default=50, help="Number of query trials for each single pattern class.")
    parser.add_argument("--positives-per-trial", type=int, default=5, help="Multi-label positive gallery maps per trial.")
    parser.add_argument("--negatives-per-trial", type=int, default=95, help="No-query-label negative gallery maps per trial.")
    parser.add_argument(
        "--proposal-mode",
...
def run_experiment(args: argparse.Namespace) -> Dict[str, Path]:
    ...
    positive_indices_by_class = _multi_label_positive_indices(labels)
    negative_indices_by_class = _no_query_label_negative_indices(labels)
    class_indices = _evaluated_class_indices(args, positive_indices_by_class)
    _validate_class_pools(
        single_indices_by_class,
        positive_indices_by_class,
        negative_indices_by_class,
        args,
        class_indices,
    )
    ...
    for class_idx in class_indices:
        ...
        for trial_idx, query_pos in pbar:
            ...
            positive_positions = _sample_positions(
                positive_indices_by_class[class_idx],
                count=args.positives_per_trial,
                rng=rng,
            )
            negative_positions = _sample_positions(
                negative_indices_by_class[class_idx],
            ...
            trial_rows.append({
                ...
                "query_class": class_name,
                "query_source_map_id": int(original_ids[query_pos]),
                "positive_label_counts": json.dumps(_label_counts(labels, positive_positions), ensure_ascii=False, sort_keys=True),
                "negative_label_counts": json.dumps(_label_counts(labels, negative_positions), ensure_ascii=False, sort_keys=True),
                **trial_metrics,
            })
    ...
    metrics = _metrics(trial_rows, args, class_indices)
...
def _multi_label_positive_indices(labels: np.ndarray) -> dict[int, np.ndarray]:
    multi = labels.sum(axis=1) > 1
    return {
        class_idx: np.flatnonzero(multi & (labels[:, class_idx] == 1)).astype(np.int64)
        for class_idx in range(len(CLASS_NAMES))
    }


def _no_query_label_negative_indices(labels: np.ndarray) -> dict[int, np.ndarray]:
    return {
        class_idx: np.flatnonzero(labels[:, class_idx] == 0).astype(np.int64)
        for class_idx in range(len(CLASS_NAMES))
    }
...
def _evaluated_class_indices(args: argparse.Namespace, positive_indices_by_class: dict[int, np.ndarray]) -> List[int]:
    selected = _selected_class_indices(args.class_name)
    if args.class_name is not None:
        return selected
    return [idx for idx in selected if len(positive_indices_by_class[idx]) > 0]


def _validate_class_pools(
    single_indices_by_class: dict[int, np.ndarray],
    positive_indices_by_class: dict[int, np.ndarray],
    negative_indices_by_class: dict[int, np.ndarray],
    args: argparse.Namespace,
    class_indices: Sequence[int],
) -> None:
    if not class_indices:
        raise ValueError("No classes have multi-label positives for this experiment.")
    missing = [CLASS_NAMES[idx] for idx in class_indices if len(single_indices_by_class[idx]) == 0]
    if missing:
        raise ValueError(f"No single-class samples for: {', '.join(missing)}")
    too_few_positives = {
        CLASS_NAMES[idx]: len(positive_indices_by_class[idx])
        for idx in class_indices
        if len(positive_indices_by_class[idx]) < args.positives_per_trial
    }
    if too_few_positives:
        raise ValueError(f"Not enough multi-label positives containing the query class: {too_few_positives}")
    too_few_negatives = {
        CLASS_NAMES[idx]: len(negative_indices_by_class[idx])
        for idx in class_indices
        if len(negative_indices_by_class[idx]) < args.negatives_per_trial
    }
    if too_few_negatives:
        raise ValueError(f"Not enough negatives without the query class: {too_few_negatives}")


def _sample_positions(pool: np.ndarray, count: int, rng: np.random.Generator) -> np.ndarray:
    return rng.choice(pool, size=count, replace=len(pool) < count).astype(np.int64)
...
def _candidate_records(
    ...
    for pos in positions:
        pos = int(pos)
        label_names = _label_names(labels[pos])
        class_name = "+".join(label_names)
        records.append({
...
def _rank_candidates(query: GridMaps, candidates: List[Dict], args: argparse.Namespace) -> List[Dict]:
    ...
    for candidate in candidates:
        ...
        ranked.append({
            ...
            "candidate_label_count": int(candidate["label_count"]),
...
def _metrics(trial_rows: List[Dict], args: argparse.Namespace, class_indices: Sequence[int]) -> Dict:
    ...
    skipped_classes = [
        CLASS_NAMES[idx]
        for idx in _selected_class_indices(args.class_name)
        if idx not in set(class_indices)
    ]
    ...
    return {
        "dataset": "Mixed38K",
        "task": "single_to_multi_retrieval",
        "positive_definition": "candidate label set contains query class and has more than one label",
        "negative_definition": "candidate label set does not contain query class",
        "primary_metric": "top5_positive_hit_rate",
        "proposal_mode": args.proposal_mode,
        ...
        "top5_positive_hit_rate": overall.get("hit_at_5", 0.0),
        "top5_positive_recall": overall.get("recall_at_5", 0.0),
        "overall": overall,
        "per_class": per_class,
    }
...
def _label_counts(labels: np.ndarray, positions: np.ndarray) -> Dict[str, int]:
    counts = {class_name: 0 for class_name in CLASS_NAMES}
    for pos in positions:
        for label_idx in np.flatnonzero(labels[int(pos)].astype(np.int32) == 1):
            counts[CLASS_NAMES[int(label_idx)]] += 1
    return {class_name: count for class_name, count in counts.items() if count > 0}


def _label_names(label_row: np.ndarray) -> List[str]:
    return [CLASS_NAMES[int(idx)] for idx in np.flatnonzero(label_row.astype(np.int32) == 1)]
