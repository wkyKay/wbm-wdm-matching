...
`arc` 基于 `compact` 的径向 ring band 和 residual component 逻辑，但不先生成 full-ring token。它先对原始缺陷 mask 做一次受有效晶圆区域约束的 `3×3` closing 作为 arc/ring 轮廓证据，因此可以捕捉有 1 个像素断裂的环或弧；token 的 `pixels`、面积、质量、几何统计和描述子仍只来自原始缺陷格，不把 closing 补出的桥接像素计入真实缺陷。
...
PYTHONPATH=wbm-wdm-matching python3 -m match.scripts.main \
  ...
  --mode count-partial \
  --proposal-mode arc \
  --identifier AF00138_arc
```

`arc` 的角向断裂限制为 1 个像素、最多 1 处断裂；覆盖率达到约 0.65 时标为 `edge_ring`，否则满足最小覆盖率的候选标为 `ring_arc`。拒绝原因会记录在 `proposal_debug["..."]["arc_detection"]` 中，并保留 `allowed_gap_cells`、`max_gap_count`、`ring_arc_gap_count` 和 `ring_arc_max_gap_cells` 等调试字段。原 `compact` 行为不变，适合直接做 A/B 实验。

### 1.1.3 切向断裂环 proposal
...
### 1.1.4 arc-ring-residual proposal

`arc-ring-residual` 从 `arc` 模式的 ring 检测中间结果（radial band → 8-connected → 1-pixel gap bridging → 20% gap-ratio → top-5 groups）出发，对每个 band group 独立进行保/弃筛选，不做进一步拆分。筛选条件：

1. **圆心角 >= 45°**（72 个 5° 扇形 bins 中被该 group 覆盖的 bin 数 ≥ 9）
2. **接触比 <= 40%**：
   - 弧长 = 角度跨度（弧度）× group 平均半径（以 wafer 中心为圆心）
   - 接触像素数 = group 中有 >=1 个 8-neighbor 落在父连通区域（parent component 减去 group 自身）的像素数
   - 接触比 = 接触像素数 / 弧长；若 > 40% 则说明 arc 大部分与父 blob 黏连，不应拆开 → 丢弃

通过的 group 各自成为独立的 arc token（`ring_band` 或 `ring_arc_band`），其 raw defect 像素从图谱中扣除后，残差部分做普通 8-连通 component token 提取。Arc token 必须保留；component token 按 √mass + √area 排序后填入剩余 top-k 槽位。
...
PYTHONPATH=wbm-wdm-matching python3 -m match.scripts.main \
  ...
  --identifier AF00138_arc_ring_residual
```

该模式复用了 `arc` 的中间数据结构（`band_groups_raw`），依赖 `--proposal-mode arc` 的完整 debug 输出；因此只在 `mixed38k_proposal_check.py` 中实现并验证，不通过 production `main.py` 调用。
...
match/
  core/
    ...
    local_matching/        # count-partial 局部匹配
      ...
      proposal.py          # token 提案生成（cc / compact / arc / tangential-ring / sparse-density）
...
从 WBM 和 WDM 的 mask 中提取若干局部区域作为 token，每个 token 代表一个有意义的缺陷聚集区。支持五种模式：
...
- **Compact**：先提取边缘环状 token（radial histogram 检测环形密集带），再从残差中提取 component token（分类为 blob / line / central / irregular），按类型多样性选取
  ...
  - 小图 ring-aware 默认使用最多 24 个角度扇区、最少 6 个 ring-band cell、最少 0.10 的角度覆盖率、最多 0.18 的径向标准差和最多 0.60 的缺陷覆盖率；较大图维持原有的 72 扇区与更严格阈值。`ring_arc` fallback 的最小角向覆盖率为 30 度。可通过 `--ring-min-area`、`--ring-edge-r-min`、`--ring-band-width`、`--ring-min-angular-coverage`、`--ring-angular-bins`、`--ring-max-radial-std`、`--ring-max-defect-ratio`、`--ring-min-edge-defect-fraction` 显式调节
- **Arc**：复用 Compact 的径向 band 与 residual component 选择逻辑，但 arc/ring 检测先对原始 mask 做一次 valid-constrained `3×3` closing，允许 1 个像素、最多 1 处断裂。closing 只作为轮廓证据；token 的真实像素、面积和质量仍只来自原始缺陷格。覆盖率达到约 0.65 时标为 `edge_ring`，否则满足 30 度以上覆盖的候选标为 `ring_arc`。
...
```
moment_sim   = contrast(moment_sim_raw,   floor=0.60, gamma=2.0)
geometry_sim = contrast(geometry_sim_raw, floor=0.45, gamma=1.5)
shape_sim    = 0.75 × moment_sim + 0.25 × geometry_sim
```

`moment_sim_raw` 和 `geometry_sim_raw` 会写入 token/map match 日志，便于调参时对比原始分布和校准后分布。校准后的 `shape_sim` 低于 `MIN_SHAPE_SIM_FOR_MATCH=0.30` 时，该 token pair 直接置零。
...
另外还有一个硬尺度门控：`--token-scale-ratio-min` 默认 `0.20`，要求面积比、PCA 长轴比、PCA 短轴比都不低于该阈值，否则该 token pair 直接记为 0 分。设为 `0` 可关闭该门控。
